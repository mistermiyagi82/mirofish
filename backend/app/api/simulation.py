...
def prepare_simulation():
    ...
    4. Intelligently generate simulation config via LLM (with retry mechanism)
    ...
    try:
        ...
        # Check if preparation is already complete (to avoid redundant generation)
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "preparing",
                "message": "Preparation task started, use /api/simulation/prepare/status to query progress",
...
def get_prepare_status():
    ...
    try:
        ...
        # If simulation_id is provided, check if preparation is already complete
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
        ...
        if not task:
            # Task not found, but if simulation_id is provided, check if preparation is already complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
                        ...
                        "data": {
                            ...
                            "status": "ready",
                            "progress": 100,
                            "message": "Task already complete (preparation exists)",
                            "already_prepared": True,
                            "prepare_info": prepare_info
                        }
                    })
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    and returns the newest one (sorted by created_at) if multiple exist.
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    - Returns extra metadata (e.g., file modification time, whether generation is ongoing)
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if the simulation is running or completed, it will be stopped and run logs cleared first
        ...
        - Useful for scenarios where re-running the simulation is needed
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": "Simulation is currently running, please call /stop to stop it first, or use force=true to force a restart"
                            }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    Used to display real-time activity in the frontend
    ...
    try:
        ...
        # recent_actions shows only the content of both platforms for the current latest round
...
def get_simulation_timeline(simulation_id: str):
    ...
    Used for the frontend progress bar and timeline view
...
def get_agent_stats(simulation_id: str):
    ...
    Used in the frontend to display Agent activity rankings, action distribution, etc.
    """
    try:
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (enters command-waiting mode after the simulation loop completes)
...
def close_simulation_env():
    ...
    whereas this endpoint lets the simulation gracefully shut down the environment and exit.
