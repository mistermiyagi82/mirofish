...
A. **Fallback types (must include, placed as the last 2 in the list)**:
   - `Person`: The fallback type for any individual person. When a person does not belong to another more specific person type, they are classified here.
   - `Organization`: The fallback type for any organization. When an organization does not belong to another more specific organization type, it is classified here.
...
- The text will contain various characters, such as "primary school teachers", "passersby", "some netizens"
...
- Person: Any individual (used when they do not belong to the above specific types)
