...
Employs a step-by-step generation strategy to avoid failures from generating excessively long content at once:
...
CHINA_TIMEZONE_CONFIG = {
    ...
    "activity_multipliers": {
        ...
        "night": 0.5       # Activity declining late at night
    }
}
...
class AgentActivityConfig:
    ...
    # Influence weight (determines probability that posts are seen by other agents)
...
class TimeSimulationConfig:
    ...
    # Time represented per round (simulated minutes) - default 60 minutes (1 hour), accelerating time flow
...
class SimulationConfigGenerator:
    ...
    to automatically generate the optimal simulation parameter configuration
...
- User base is Chinese; the schedule should follow Beijing time habits
- Almost no activity from 0-5 AM (activity multiplier 0.05)
- Gradually increasing activity from 6-8 AM (activity multiplier 0.4)
- Moderate activity during work hours 9-18 (activity multiplier 0.7)
- Evening peak from 19-22 (activity multiplier 1.5)
- Decreasing activity after 23:00 (activity multiplier 0.5)
- General pattern: low activity in early morning, increasing in the morning, moderate during work hours, peak in the evening
- **Important**: The example values below are for reference only. Adjust the specific time slots based on the nature of the event and characteristics of the participant groups.
  - For example: a student population may peak at 21-23; media accounts are active all day; official institutions only operate during work hours
  - For example: a breaking news event may lead to late-night discussions, so off_peak_hours can be shortened
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        Matches the best agent_id for each post based on its poster_type
        """
        ...
        # Track the used agent index for each type to avoid reusing the same agent
        ...
        for post in event_config.initial_posts:
            ...
            if matched_agent_id is None:
                ...
                if agent_configs:
                    # Sort by influence weight and select the highest
...
- **Schedule must follow Chinese daily habits**: Almost no activity from 0-5 AM, most active from 19-22 evening
...
{{
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        if entity_type in ["university", "governmentagency", "ngo"]:
            ...
            # Ordinary people: peak in the evening
            return {
                "activity_level": 0.7,
                "posts_per_hour": 0.5,
