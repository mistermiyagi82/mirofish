...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        try:
            ...
            # Set working directory to the simulation directory (databases and other files will be created here)
            # Use start_new_session=True to create a new process group, ensuring os.killpg can terminate all child processes
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        """
        Check if all enabled platforms have completed their simulation

        Platform enablement is determined by checking whether the corresponding actions.jsonl file exists
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: use the record's platform field first, otherwise use the default platform
    ...
    def register_cleanup(cls):
        ...
        Called when the Flask app starts, to ensure all simulation processes are terminated on server shutdown
        """
        ...
        # In Flask debug mode, only register cleanup in the reloader child process (the actual running process)
        # WERKZEUG_RUN_MAIN=true indicates this is the reloader child process
        # If not in debug mode, this env var is absent but cleanup still needs to be registered
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                ...
                - None: In dual-platform simulation, interview both platforms for each agent simultaneously
            timeout: Timeout in seconds
