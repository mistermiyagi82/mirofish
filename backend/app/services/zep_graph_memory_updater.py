...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        ...
        Uses a natural language description format, allowing Zep to extract entities and relationships
        ...
        # Return directly in "agent_name: activity_description" format, without simulation prefix
...
class ZepGraphMemoryUpdater:
    ...
    Activities are grouped by platform; each platform accumulates BATCH_SIZE activities before sending them in a batch to Zep.

    All meaningful actions are updated to Zep, and action_args contains full context information:
    - Original post content of liked/disliked posts
    - Original post content of reposted/quoted posts
    - Username of followed/muted users
    - Original comment content of liked/disliked comments
    """
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Per-platform activity buffers (each platform accumulates to BATCH_SIZE before sending in a batch)
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        """
        Send activities to the Zep graph in a batch (merged into a single text)
    ...
    def _flush_remaining(self):
        ...
        # Then send remaining activities in each platform's buffer (even if fewer than BATCH_SIZE)
        with self._buffer_lock:
