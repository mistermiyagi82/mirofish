...
const checkPlatformsCompleted = (data) => {
  ...
  // Check completion state for each platform
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true

  // Check if all enabled platforms have completed
  // Determine if a platform is enabled by actions_count > 0 or was ever running
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // No auto-scroll — let users browse the timeline freely
