...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll to keep button at the same viewport position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract stats — matches "相关预测事实: X条" format
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Parse each individual's selection rationale
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g. 1. **校友_345（index=1）**: as a Wuhan University alumnus...
        ...
        // Format 2: - 选择name（index X）: rationale
        // e.g. - 选择家长_601（index 0）: as a parent representative...
        if (!headerMatch) {
        ...
        // e.g. - **家长_601（index 0）**: as a parent representative...
        if (!headerMatch) {
        ...
        if (name) {
          ...
          // Continuation lines of rationale (exclude trailing summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // Special handling when first question is prefixed with "1."
      ...
      // Extract answer — split by Twitter and Reddit
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          // No per-platform markers (very old format) — use whole answer
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Attempt to extract edge info (if present)
    ...
    // Attempt to extract node info (if present)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Fallback to "number." format if nothing matched
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // If no numbers found or only one, return the whole text
      if (matches.length <= 1) {
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: return full answer for first question, empty for others
      return qIdx === 0 ? answer : ''
    }
    
    // Check whether a question has answers from both platforms (filtering out placeholders)
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        // Q&A Conversation Thread — one question, one answer
...
// Currently active step (for top-bar display)
...
const renderMarkdown = (content) => {
  ...
  // Handle lists — support nested sub-lists
  ...
  // Handle line breaks — double newline becomes paragraph break, single becomes <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Remove <br> between closing block element and next paragraph/div
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: when single-item <ol> elements are separated by paragraph content, maintain incrementing numbers
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic is handled in a unified nextTick after the loop
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If complete, scroll to top; otherwise scroll to bottom to follow latest logs
            if (isComplete.value) {
...
// Extract final answer content — parses section content from an LLM response
const extractFinalContent = (response) => {
  ...
  // Try finding content after "最终答案:" (Chinese final answer label)
  ...
  // If content is long and contains markdown, try removing thought prefix and returning the rest
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove the "Thought:" reasoning preamble
