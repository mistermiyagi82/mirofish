            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After documents are uploaded, the LLM analyzes their content and automatically generates an ontology structure suitable for public-opinion simulation (entity types + relation types).
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Description</div>
                <div class="detail-content">
                  Using the generated ontology, documents are chunked and fed to the Zep API to build a knowledge graph by extracting entities and relations.
                </div>
              </div>
...
const initProject = async () => {
  ...
  if (paramProjectId === 'new') {
    // New project: fetch pending upload data from the store
...
const startGraphPolling = () => {
  ...
  // Then auto-fetch every 10 seconds
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        // Update progress display to reflect completion
        buildProgress.value = {
          progress: 100,
        ...
        if (projectResponse.success) {
          ...
          // Final load of the full graph data
...
const renderGraph = () => {
  ...
  // Draw edges (with a wide transparent hit area for click detection)
  ...
  // Click on empty area to close the detail panel
  svg.on('click', () => {
    closeDetailPanel()
  })

  simulation.on('tick', () => {
    // Update positions of all edge lines (visible + transparent hit area)
