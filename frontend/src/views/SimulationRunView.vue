...
const handleGoBack = async () => {
  ...
  try {
    // Attempt graceful shutdown of the simulation environment first
...
const loadGraph = async (graphId) => {
  // During simulation, suppress the full-screen loading indicator on auto-refresh to avoid flickering.
...
const startGraphRefresh = () => {
  ...
  addLog('Graph live refresh started (every 30s)')
...
onMounted(() => {
  ...
  // Log maxRounds config (value was already read from query params at init time)
  if (maxRounds.value) {
